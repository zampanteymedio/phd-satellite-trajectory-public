...
def main(argv):
    parser = argparse.ArgumentParser(description='Run A2C on the perigee raising continuous 1D environment.')
...
def process(layers, case_number, steps, envs, verbose):
    name = f"c1a_A2C_{str(layers).replace(' ', '')}_{case_number}"
    print(f"Case: {name}")
    env = make_vec_env('PerigeeRaising-Continuous1D-v0', n_envs=envs, wrapper_class=lambda x: wrap(x))
